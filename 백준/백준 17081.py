...
def bossbattle(po) :
    ...
    print("my status : ", mlevel, mREM, mCUR, mATT, mDEF, mW, mA)
...
def battle(po) :
    ...
    while True :
        if startattack != 0 :
            monH -= max(1, (startattack - monA))
            print("My Hit! :", max(1, (startattack - monA)))
            startattack = 0
        else :
            monH -= max(1, ((mATT + mW) - monA))
            print("My Hit! :", max(1, ((mATT + mW) - monA)))

        print(monstername[po], "HP", monH)
...
def move(com, now, board) :
    ...
    if 1 <= cx < M+1 and 1 <= cy < M+1 and board[cy-1][cx-1] != '#' :
        return (cx, cy)
    else :
        return (x, y)
...
for i in command :
    ...
    if board[y-1][x-1] == '.' : ## nothing
        ...
        if itemT[po] == 'W' :
            print("get weapon! : ", itemS[po])
            mW = itemS[po]
        elif itemT[po] == 'A' :
            print("get armor! : ", itemS[po])
            mA = itemS[po]
        elif itemT[po] == 'O' :
            print("get O! : ", itemS[po])
